...
STEER_GAIN = 75   # Steering sensitivity. Higher values make the car turn more aggressively.
CENTERING_GAIN = 1.2  # How strongly the car corrects its position toward the center of the track.
...
def calculate_steering(S):
    multipler = 1
    if isCorner(getCornerSensor(S)):
        multipler = 2

    steer = (S['angle'] * STEER_GAIN / PI) - (S['trackPos'] * CENTERING_GAIN) * multipler
...
def apply_brakes(S):
    brake = 0.0
    min_reading = getCornerSensor(S)
    
    if abs(S['angle']) > BRAKE_THRESHOLD:
        brake = ((abs(S['angle']) / PI) / BRAKE_THRESHOLD)
